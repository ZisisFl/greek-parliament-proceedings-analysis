...
import auth.dws.bigdata.common.RAKEStrategy.RAKEStrategy

import scala.collection.mutable
import scala.collection.mutable.{Map => MMap}

// RAKE implementation taken from https://github.com/mdymczyk/spark-miner/blob/master/src/main/scala/org/apache/spark/ml/feature/RAKE.scala
  ...
  def toRAKESeq(doc: String): Seq[Seq[String]] = {
    ...
    doc.foreach { currChar =>
      if (wordDelims.contains(currChar)) {
        if (!stopwords.contains(currWord.toString().toLowerCase)) {
        ...
        if (!stopwords.contains(currWord.toString().toLowerCase) && currWord.nonEmpty) {
